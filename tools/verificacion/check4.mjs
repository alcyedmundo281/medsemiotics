...
// Todos los selectores de clase, a cualquier nivel de indentación.
const present = new Set();
for (const m of css.matchAll(/^\s*\.((?:[^\s{,:]|\\.)+)/gm)) {
  present.add(m[1].replace(/\\(.)/g, '$1'));
}
// Variantes tipo .dark\:bg-x:is(.dark *) y .hover\:x:hover
for (const m of css.matchAll(/^\s*\.((?:[^\s{,]|\\.)+?)(?::is\(|:hover|:focus|:active|:checked|:disabled|::before|::after|:nth|\s|\{)/gm)) {
  present.add(m[1].replace(/\\(.)/g, '$1'));
}
